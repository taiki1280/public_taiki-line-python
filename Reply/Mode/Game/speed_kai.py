...
def run(self):
    self.RUNNING_ID_DICT[self.running_id]["ゲームモード"].setdefault(
        "スピード改", {"ステップ": 0, "対戦回数": 0, "バトルゾーン": [], "参加者": {}})
    ...
    # 初期設定
    if self.step == 0:
        print("loaded speed_kai")
        from ..game import cannot_play
        if cannot_play(self, 3, ["CPUとやっても絶対的に勝てないからな。"]):
            return
        self.reply(
            self.game_msg
            + [
                "このゲームはトランプゲームであるスピードを数字のみに置き換えて作成したものです。",

                "「ルール」\n"
                + "多分スピード知らん人はおらんやろ！\n"
                + "もし知らなければ、すぐさまggるなりその辺の人に聞くなりしてから出直してきてくれ！",

                "このゲームは通常２人で行うゲームです。２人までしか参加を受け付けません。\n"
                + "まずは、ゲームをする意思を示して欲しい。\n"
                + "「参加」と入力すると準備ができます。\n"
                + "参加をしたい２人は「参加」と入力してください。"
            ]
        )
